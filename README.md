...
## Summary: 
Code refactoring means restructuring the existing code, in a way that improves the internal structure but doesn’t change external behavior. The advantage of refractoring code is you make the process more efficient in the long term. We want to transform messy or complicated code into clean code is code that is easy to read, understand, and maintain. If we invest the time to do this initially, we can minimize the risk of the program lagging or crashing, which is beneficial when doing data analysis. Crashing isn't much of an issue with our data right now, but if we wanted to analyze thousands of stocks, we could do so easily with our refractored code. Refractoring code makes the data analysis scalable.

A disadvantage of refractoring code is any time you change code you have the potential to introduce new bugs. I had tro try to work on AllStocksAnalysisRefactored() for a number of hours before I got the same output as AllStocksAnalysis(). If we just needed to get the output for this data one time, it may not be worth the time it takes to refractor. The decision to not refractor code can be considered effecient in the short term. 

With regards to these specific macros, AllStocksAnalysisRefactored() only forces the program to read the entire dataset once, whoch saves time an bandwith. If the data we wanted to analyze was hundreds of thousands of rows, the difference would be much more important than waiting an extra 5 seconds. It reduces the likelihood that the code will break. An advantage of the original AllStocksAnalysis() is that if the data was not structured the way we originally interpreted it, it would still produce the same result. That is because the nested for loop reads the data in its entirety for each ticker. If there was an out of place *AY* ticker in row 2,000, the output would not be affected because thif statement 5a would be satisfied and that volume would be added to total volume. Also, even if the intital **endingPrice** was in row 252, once j reaches 2,000, that row would also satisfy the condition and replace the prior **endingPrice** in the final output. This would not work in AllStocksAnalysisRefactored(). In AllStocksAnalysisRefactored(), if there were an out of place ticker, it would be read as a new ticker because the ticker in row 1,999 would not equal the row ticker in row 2,000 so the If Statements 3c and 3d would both be satisfied. Then VBA would read the stray *AY* ticker in row 2,000 as it's own tickerIndex, causing us to miss the output of the twelth ticker *VSLR* and our analysis to be completely messed up. The decision to refractor or not refractor depends on the dataset, the question we are tryong to solve, and the time we have to answer that question. 
